...
namespace DataAccessLayer
{
    ...
    public sealed class SQLHelper : ISQLHelper
    {
        ...
        // instances from being created with "new SqlDALResourceAdapter()"
        public SQLHelper()
        {
        }

        /// <summary>
        /// This method is used to attach array of IDbDataParameters to a IDbCommand.
        /// 
        ...
        private static void PrepareCommand(IDbCommand command, IDbConnection connection, IDbTransaction transaction, CommandType commandType, string commandText, IDbDataParameter[] commandParameters, out bool mustCloseConnection)
        {
            ...
            //IGlobalConfiguration productConfig = ToolkitConfiguration.GetProductConfigurationClass();
            ////TODO:(Configuration) Remove this check of null in 'if' and remove else block after all products uses its own strongly type configuration class.  
            //if ( productConfig != null )
            //{
            //    command.CommandTimeout = productConfig.DatabaseTimeout;
            //}
            //else
            //{

            // command.CommandTimeout = int.Parse( c360.Toolkit.Util.GetWebConfigSetting( "DatabaseTimeout", "300" ) );
        ...
        public DataSet ExecuteDataset(string connectionString, CommandType commandType, string commandText, params IDbDataParameter[] commandParameters)
        {
            ...
            using (IDbConnection connection = new SqlConnection(connectionString))
            {
                ...
                catch (Exception)
                {
                    // S1570
                    // [
                    // Implemented new logging and exception policy.
                    throw;
        ...
        /// <summary>
        /// This enum is used to indicate whether the connection was provided by the caller, or created by SqlDALResourceAdapter, so that
        /// we can set the appropriate CommandBehavior when calling ExecuteReader()
        /// </summary>
        private enum IDbConnectionOwnership
        {
            /// <summary>Connection is owned and managed by SqlDALResourceAdapter</summary>
            Internal,
        ...
        /// <param name="commandText">The stored procedure name or T-SQL command</param>
        /// <param name="commandParameters">An array of IDbDataParameters to be associated with the command or 'null' if no parameters are required</param>
        /// <param name="connectionOwnership">Indicates whether the connection parameter was provided by the caller, or created by SqlDALResourceAdapter</param>
        /// <returns>IDataReader containing the results of the command</returns>
        private static IDataReader ExecuteReader(IDbConnection connection, IDbTransaction transaction, CommandType commandType, string commandText, IDbDataParameter[] commandParameters, IDbConnectionOwnership connectionOwnership)
        {
            if (connection == null) throw new ArgumentNullException("connection");
        ...
        /// <remarks>
        /// e.g.:  
        ///  IDataReader dr = ExecuteReader(connString, CommandType.StoredProcedure, "GetOrders", new SqlParameter("@prodid", 24));
        /// </remarks>
        /// <param name="connectionString">A valid connection string for a IDbConnection</param>
        /// <param name="commandType">The CommandType (stored procedure, text, etc.)</param>
        /// <param name="commandText">The stored procedure name or T-SQL command</param>
        ...
        public IDataReader ExecuteReader(string connectionString, CommandType commandType, string commandText, params IDbDataParameter[] commandParameters)
        {
            ...
            try
            {
                ...
                dataReader = ExecuteReader(connection, null, commandType, commandText, commandParameters, IDbConnectionOwnership.Internal);
            }
            catch (Exception)
            {
        ...
        /// <remarks>
        /// e.g.:  
        ///  IDataReader dr = ExecuteReader(conn, CommandType.StoredProcedure, "GetOrders", new SqlParameter("@prodid", 24));
        /// </remarks>
        /// <param name="connection">A valid IDbConnection</param>
        /// <param name="commandType">The CommandType (stored procedure, text, etc.)</param>
        /// <param name="commandText">The stored procedure name or T-SQL command</param>
        ...
        /// <remarks>
        /// e.g.:  
        ///   IDataReader dr = ExecuteReader(trans, CommandType.StoredProcedure, "GetOrders", new SqlParameter("@prodid", 24));
        /// </remarks>
        /// <param name="transaction">A valid IDbTransaction</param>
        /// <param name="commandType">The CommandType (stored procedure, text, etc.)</param>
        /// <param name="commandText">The stored procedure name or T-SQL command</param>
        ...
        {
            // Pass through the call providing null for the set of IDbDataParameters
            return ExecuteXmlReader(connection, commandType, commandText, (IDbDataParameter[])null);
        }
        ...
        /// <param name="commandParameters">An array of SqlParamters used to execute the command</param>
        /// <returns>An XmlReader containing the resultset generated by the command</returns>
        public XmlReader ExecuteXmlReader(IDbConnection connection, CommandType commandType, string commandText, params IDbDataParameter[] commandParameters)
        {
            if (connection == null) throw new ArgumentNullException("connection");
        ...
        {
            // Pass through the call providing null for the set of IDbDataParameters
            return ExecuteXmlReader(transaction, commandType, commandText, (IDbDataParameter[])null);
        }
        ...
        /// <param name="commandParameters">An array of SqlParamters used to execute the command</param>
        /// <returns>An XmlReader containing the resultset generated by the command</returns>
        public XmlReader ExecuteXmlReader(IDbTransaction transaction, CommandType commandType, string commandText, params IDbDataParameter[] commandParameters)
        {
            if (transaction == null) throw new ArgumentNullException("transaction");
            if (transaction != null && transaction.Connection == null) throw new ArgumentException("The transaction was rollbacked or commited, please provide an open transaction.", "transaction");
        ...
        public IDbCommand CreateCommand(IDbConnection connection, string spName, params string[] sourceColumns)
        {
            if (connection == null) throw new ArgumentNullException("connection");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName");
    ...
    public sealed class SqlHelperParameterCache
    {
        ...
        //Since this class provides only static methods, make the default constructor private to prevent 
        //instances from being created with "new SqlDALResourceAdapterParameterCache()"
        ...
        private static IDbDataParameter[] DiscoverSpParameterSet(IDbConnection connection, string spName, bool includeReturnValueParameter)
        {
            ...
            IDbDataParameter[] discoveredParameters = new SqlParameter[cmd.Parameters.Count];
        ...
        {
            IDbDataParameter[] clonedParameters = new SqlParameter[originalParameters.Length];
